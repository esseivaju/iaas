## Ingresses
There are two ingresses for the deployment, one for GRPC traffic directed to envoy/triton in [triton-cluster.yaml](triton-cluster.yaml), and another ingress in [ingress.yaml](ingress.yaml) for regular http traffic to grafana / the triton controller.
